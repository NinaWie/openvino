// Naive batched-expert INT3 MoE GEMM.
//
...
//
// Every buffer is a contiguous stack of per-expert slices; the expert index is
// the third grid dimension and only ever contributes a base offset. Rows are
// padded to m_stride so all expert slices share one stride.
//
// A        int8 activations, [G, m_stride, K] row-major (dyn-quantized upstream)
// a_scale  half, [G, m_stride, K / GROUP_SIZE] per-group activation scale
// B        u3 weights, 3-bit values packed LSB-first, in the blocked layout
//          described in kernel.cl, repeated per expert (identical buffer for
//          both kernels)
// b_scale  half, [G, N, K / GROUP_SIZE] per-group weight scale
// C        half, [G, m_stride, N]
...
__kernel void int3_moe_gemm_ocl(const __global char* A,
    ...
    const uint expert = (uint)get_global_id(2);
