"""INT3 (u3) weight-decompression GEMM for OpenVINO GPU.

The Qwen3.6-35B-A3B int3 model stores 160 weight tensors as u3 with a per-group
f16 scale (group_size=128 along K) and a single scalar i8 zero point of 4.
OpenVINO's OpenCL path has no u3 support at all today, so these layers fall back
to oneDNN's reference implementation. This task develops the OpenCL kernel:
u3 weights, int8 activations, int32 accumulation, f16 output.

Both kernels consume the identical buffers, including the blocked u3 weight
layout described in kernel.cl. Correctness is checked against a numpy ground
truth computed in exact integer arithmetic (float32 matmul is exact here because
|A| <= 127 and |W - zp| <= 4, so every partial sum stays far below 2^24).
"""
...
def make_data(
    ...
    weights = rng.integers(0, 8, size=(n_size, k_size), dtype=np.uint8)

    # Scales sized so that the f16 output stays in a comfortable range.
    a_scale = rng.uniform(0.005, 0.02, size=(m_pad, groups)).astype(np.float16)
    b_scale = rng.uniform(0.005, 0.02, size=(n_size, groups)).astype(np.float16)
...
def compute_expected(data: dict) -> np.ndarray:
    ...
    out = np.zeros((m_size, n_size), dtype=np.float32)
    for g in range(groups):
        k0, k1 = g * GROUP_SIZE, (g + 1) * GROUP_SIZE
        acc = acts[:, k0:k1] @ centered[:, k0:k1].T  # exact integers in float32
        out += acc * a_scale[:, g : g + 1] * b_scale[None, :, g]
    return out
...
def to_device(queue: cl.CommandQueue, data: dict) -> tuple[tuple[Any, ...], cl.Buffer]:
    ...
    c_buf = cl.Buffer(ctx, cl.mem_flags.READ_WRITE, size=data["M_pad"] * data["N"] * 2)

    args = (a_buf, a_scale_buf, b_buf, b_scale_buf, c_buf, np.int32(data["M"]))
    return args, c_buf
...
class TestInt3GemmOCL(TestBase):
    """Optimize the u3 x int8 weight-decompression GEMM."""
    ...
    def test_correctness(self, use_reference, ocl_queue, data_for_test):
        ...
        got = np.empty((data["M_pad"], data["N"]), dtype=np.float16)
    ...
    def test_performance(self, use_reference, ocl_queue, measure_runtime, data_for_test):
        """Benchmark the decode, MoE and dense-prefill regimes of the int3 model."""
        data = data_for_test
